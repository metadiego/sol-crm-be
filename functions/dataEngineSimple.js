...
function getConnection() {
  ...
  if (process.env.SOLANA_NETWORK == 'devnet') {
    return new solana.Connection("https://sparkling-divine-frog.solana-devnet.quiknode.pro/7e35f0f6419ac6435467b552c16ee00567d5a56e/");
  }
  return new solana.Connection("https://patient-delicate-darkness.solana-mainnet.quiknode.pro/e845f8680b317cebfd5feb724d9200660f330c93/");
}

// TODO: add some way to test the Solana Network Connection
...
async function GetHolderSnapshot(connection, mintId) {
...
async function GetHolderTransactionData(connection, publicKey, data) {
  ...
  const holderData = { };
  for (const [mintId, value] of Object.entries(data)) {
    const mintAcquisitionDate = transactionDates.get(mintId)?.date;
    holderData[mintId] = {
      amount: value.amount,
      mintAcquisitionDate,
    };
...
const GetHolderData = async (mintIds) => {
  ...
  Object.entries(holderSnapshots).map(([publicKey, data]) => {
    holderSnapshotsWithDataPromises.push(GetHolderTransactionData(connection, publicKey, data));
  });
